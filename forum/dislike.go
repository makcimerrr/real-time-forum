...
func DislikeDiscussion(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		discussionID := r.URL.Path[len("/dislike/"):]
		...
		if disliked {
			...
			// Ajoutez un dislike
			_, err = db.Exec("INSERT INTO dislikes (discussion_id, username) VALUES (?, ?)", discussionIDInt, username)
			if err != nil {
				http.Error(w, "Internal Server Error", http.StatusInternalServerError)
				return
			}
...
func CheckIfUserDislikedDiscussion(username string, discussionID int) (bool, error) {
	db, err := sql.Open("sqlite", "database/data.db")
	if err != nil {
		return false, err
	}
	...
	var exists bool
	err = db.QueryRow("SELECT EXISTS(SELECT 1 FROM dislikes WHERE discussion_id = ? AND username = ?)", discussionID, username).Scan(&exists)
	if err != nil {
		return false, err
	}
...
func CheckNumberOfDislikesForDiscussion(discussionID int) (int, error) {
	db, err := sql.Open("sqlite", "database/data.db")
	if err != nil {
		return 0, err
	}
	...
	var likeCount int
	err = db.QueryRow("SELECT COUNT(*) FROM dislikes WHERE discussion_id = ?", discussionID).Scan(&likeCount)
	if err != nil {
		return 0, err
	}
